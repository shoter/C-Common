...
namespace Common.Xml
{
    ...
    public class SerializableDictionary: Dictionary<string, object>, IXmlSerializable
    {
        ...
        /// </summary>
        private static readonly XmlSerializer keySerializer = new XmlSerializer(typeof(KeyWithType));
        ...
        private void ReadItem(XmlReader reader)
        {
            ...
            try
            {
                var keyType = this.ReadKeyWithType(reader);

                this.Add(keyType.Key, this.ReadValue(reader, keyType.Type));
            }
            finally
            {
                reader.ReadEndElement();
        ...
        private KeyWithType ReadKeyWithType(XmlReader reader)
        {
            reader.ReadStartElement(this.KeyTagName);
            try
            {
                return (KeyWithType)keySerializer.Deserialize(reader);
            }
            finally
            {
                reader.ReadEndElement();
        ...
        private void WriteItem(XmlWriter writer, KeyValuePair<string, object> keyValuePair)
        {
            writer.WriteStartElement(this.ItemTagName);
            try
            {
                this.WriteKey(writer, keyValuePair.Key, keyValuePair.Value.GetType());
        ...
        private void WriteKey(XmlWriter writer, string key, Type type)
        {
            writer.WriteStartElement(this.KeyTagName);
            try
            {
                var keyType = new KeyWithType(key, type);
                keySerializer.Serialize(writer, keyType);
            }
            finally
            {
                writer.WriteEndElement();
        ...
        private void WriteValue(XmlWriter writer, object value)
        {
            ...
            try
            {
                GetXmlSerializer(value.GetType()).Serialize(writer, value);
            }
            finally
            {
                writer.WriteEndElement();
